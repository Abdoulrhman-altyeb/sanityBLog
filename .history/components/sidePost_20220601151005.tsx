    ...
    <Link href={`/post/${single.slug.current}`}>
        <div className='flex shadow-custom justify-between cursor-pointer mb-10 p-2 overflow-hidden group'>
          ...
          <div className='overflow-hidden'>
            <img className='h-40 max-w-6xl object-cover group-hover:scale-105 transition-transform duration-150' src={urlFor(single.mainImage.asset._ref).url()} alt='mainImage'/>
          </div>
        </div>
    </Link>
  )
